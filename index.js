  ...
  return `
    <li class='js-item-element' data-item-id='${item.id}'>
      ...
      <div class='shopping-item-controls'>
        ...
        <form class='shopping-item-edit-form'>
          <input name='shopping-item-edit-input' class='shopping-item-edit-input' type="text" placeholder="${item.name}">
          <button class='shopping-item-edit js-item-edit' type="submit">edit
          </button>
        </form>
      </div>
    </li>`;
...
/* Allows user to edit name of item and render once complete */
/* on EDITBUTTON click -> open ITEMNAME as input field */

const handleEditItemClick = function() {
  $('.shopping-item-edit-form').submit(event => {
    event.preventDefault();
    const id = getItemIdFromElement(event.currentTarget);
    const newItemName = $('.shopping-item-edit-input').val();
    const index = store.items.findIndex(item => item.id === id);
    store.items[index].name = newItemName;
    $('.shopping-item-edit-input').val('');
    render();
  });
}
...
/* shopping-item-edit js-item-edit */
